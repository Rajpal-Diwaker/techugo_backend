...
public function getTeamList($rowno,$rowperpage,$search=""){    
    $this->db->select("t1.*,(SELECT COUNT(ptTeamId)+1 FROM `tbl_project_team` WHERE ptTeamId=t1.tId AND `status` = 'active') AS teamMemberCount");
    $this->db->from('tbl_team as t1'); 
    $this->db->where('t1.status=','active');
    $query = $this->db->get();
    $result = $query->result_array();
...
public function updateTeam($postdata){
  ...
  //HTML
  if($postdata['HTML_Id']||($postdata['HTML'])){
      if(!empty($postdata['HTML_Id'])){
  ...
  //QA
  if($postdata['QA_Id']||($postdata['QA'])){
        if(!empty($postdata['QA_Id'])){
    ...
    //Designer
  if($postdata['Designer_Id']||($postdata['Designer'])){
      if(!empty($postdata['Designer_Id'])){
...
public function checkUserbandwidth($userId,$hours){    
  $sql="SELECT (CASE  WHEN (SELECT 8-(SELECT SUM(total) AS  total FROM ( (SELECT  SUM(ptdailyHours) AS total FROM `tbl_project_team` WHERE STATUS='active'  
