...
<div class="table-responsive tablewrap">
   <table id="table" class="w3-table-all">
          ...
          <button class="btn btn-info">
          <a href="<?php echo base_url('Admin/viewProject?projectId='.$value['pId']);?>">View</a></button>
          </td>
          <?php $i++;?>
          </tr>
...
<script>
 $(document).ready(function() {
        $('select[name="tStatus"]').on('change', function() {
            ...
            var productId = $(this).parent().parent().find('td').attr('id_data');
            if(status) {
                $.ajax({
                    url: "<?php echo base_url("Admin/chnagePojectStatus");?>",
                    type: "POST",
                    data: {tStatus:status,projectId:productId},
