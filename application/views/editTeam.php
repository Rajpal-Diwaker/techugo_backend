  ...
  <div class="form-group row">
      <div class="col-md-3 col-sm-4 col-xs-6 mobile-screen-form">
       ...
       <div class="col-md-3 col-sm-4 col-xs-6 mobile-screen-form">
          ...
          <input type="text"  Id="ProjectId" name="ProjectId" value="<?php echo $projectDetails['pId']; ?>" hidden/>
       ...
       <div class="col-md-3 col-sm-4 col-xs-6 mobile-screen-form">
          ...
          <input type="number" class="form-control" id="Project_R_Hours" name="Project_R_Hours"  
           value="<?php echo $getProjectManager[0]['ptdailyHours'];?>"
            placeholder="Hours" step="0.01" min="1" max="8" required/><br/>
      </div>
...
<input type="hidden" id="HTML_Id" value="<?php  echo $TeamProjectHTML['ptId']; ?>" name="HTML_Id"/>
          ...
          <input type="number" class="form-control" id="HTML_R_Hours" name="HTML_R_Hours" 
           placeholder="Hours" step="0.01" min="1" max="8" style="width: 278px;"
           value="<?php echo $TeamProjectHTML['ptdailyHours'];?>" /><br/>

           <span class="btn btn-primary HTMLRemove" 
       <?php  if(empty($TeamProjectHTML['ptId'])){ $visiblity='hidden';  }else{ $visiblity='show'; } ?>
       style="width: 30px;height: 27px;margin-left:248px;
       margin-top:-39px;visibility:<?php echo $visiblity ?>">-</span>
      ...
      <input type="hidden" id="QA_Id" value="<?php  echo $TeamProjectQA['ptId']; ?>" name="QA_Id"/>
       ...
       <div class="col-md-3 col-sm-4 col-xs-6 mobile-screen-form">
          ...
          <input type="number" class="form-control" id="QA_R_Hours" name="QA_R_Hours" 
           placeholder="Hours" step="0.01" min="1" max="8" 
           value="<?php echo $TeamProjectQA['ptdailyHours'];?>"/><br/>

          <span class="btn btn-primary QARemove" 
       <?php  if(empty($TeamProjectQA['ptId'])){ $visiblity='hidden';  }else{ $visiblity='show'; } ?>
       style="width: 30px;height: 27px;margin-left:248px;
       margin-top:-39px;visibility:<?php echo $visiblity ?>">-</span>
       ...
       <span class="btn btn-primary DesignerRemove" 
       <?php  if(empty($TeamProjectDesign['ptId'])){ $visiblity='hidden';  }else{ $visiblity='show'; } ?>
       style="width: 30px;height: 27px;margin-left:248px;
       margin-top:-39px;visibility:<?php echo $visiblity ?>">-</span>
       </div>
...
$("#IOS_R_Hours").change(function(){
    ...
    userId=$('#Ios').val();  
    timeRagechaker(hours,"IOS_R_Hours");  
    $.ajax({
            url: "<?php echo base_url('Admin/checkUserbandwidth'); ?>", 
            method:'post',
            data:{'hours':hours,'userId':userId},
...
$("#Design_R_Hours").change(function(){
    ...
    $.ajax({
            url: "<?php echo base_url('Admin/checkUserbandwidth'); ?>", 
            method:'post',
            data:{'hours':hours,'userId':userId,'projectId':projectId},
            success: function(res)
            {
                var data = $.parseJSON(res);                
                if(data.totalTime=="NOBANDWIDTH")
...
$('.DesignerRemove').on('click',function(){
  if (confirm('Are you sure you want to Remove The developer')){
        removeId=document.getElementById("Designer_Id").value;
        $.ajax({
            url: "<?php echo base_url('Admin/removeTeamUser'); ?>", 
            method:'post',
            data:{ptId:removeId},
...
$('.QARemove').on('click',function(){
  if (confirm('Are you sure you want to Remove The developer')){
        removeId=document.getElementById("QA_Id").value;
        $.ajax({
            url: "<?php echo base_url('Admin/removeTeamUser'); ?>", 
            method:'post',
            data:{ptId:removeId},
...
$('.HTMLRemove').on('click',function(){
  if (confirm('Are you sure you want to Remove The developer')){
        removeId=document.getElementById("HTML_Id").value;
        $.ajax({
            url: "<?php echo base_url('Admin/removeTeamUser'); ?>", 
            method:'post',
            data:{ptId:removeId},
